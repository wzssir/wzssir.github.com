	...
	var _trademark={
		init:function(){
			...
			if($('.trademark_step2_wrapper').length>0){
				...
				this._service_charge=parseInt($('#service_charge').val());
				this._base_class_num=parseInt($('#base_class_num').val());
				this._class_charge1=parseInt($('#class_charge1').val());
				this._class_charge2=parseInt($('#class_charge2').val());
				...
				if($('.pay_input').length>0){
					...
					$('.J_order_btn').click(function(){
						if(_switch){
							...
							$.dialog({
		    					content:'<span style="margin-bottom:10px;display:block;">Remittance Receipt：<i style="color:#ff1111;">'+_payInput+'</i></span>Your payment will be verified off line after you submitted the Remittance Number. Continue?',
		    					ok:'ok',
		    					cancel:"cancel",
		    					type:2,
		    					init:function(){
		...
		trademark_step3_handle:function(){
			...
			$('.m_checkImg_dialog .m_dialog_content').click(function(){
		...
		trademarkSteps:function(){
			...
			$('.J_steps_btn2').click(function(){
				$.dialog({
					content:'<span class="m_dialog_close">×</span>We offer a template of Power of Attorney for you to fill in, print, signed or sealed, scanned with color and revert to us by uploading (in the “Upload” step). If it takes long to make this available, we offer the template here for you to prepare in advance (and if you are not sure how to fill it, please see the <a href="poa.html" style="color:#ff4c4c;">Reference Sample</a>). You may start application once the documents are ready.<p style="text-align:center"><a href="javascript:;" class="trademark_download_link">Download</a></p>',
					type:0,
					autoHide:false,
					init:function(){
						$('.dialog_text').css({'font-size':'18px','line-height':'24px','text-align':'left'});
			...
			$('.J_steps_btn3').click(function(){
				$.dialog({
					content:'<span class="m_dialog_close">×</span>1. A clear specimen of your mark in jpg format is required. The file shall be less than 200KB, and the pixel between 400*400 and 1500*1500.<br/>2. If you prefer to upload a scan copy of specimen, it shall be scanned into a size between 5 cm*5 cm to 10 cm*10 cm in 24-colored, 300dpi-resolutioned parameters.<br/>3. You may adjust the specimenin the clipping window. Please leave a space no less than 3 mm around the specimen to avoid any defects in printing the publication or certificate of registration.',
					type:0,
					autoHide:false,
					init:function(){
						$('.dialog_text').css({'font-size':'18px','line-height':'24px','text-align':'left'});
		...
		addCheckedClass:function(){
			...
			if($('.trademark_tab_list').length<1){
				_html+="<p class='trademark_class_count'>"+
							...
							"<span class='ml68'>Official fee＋Service fee: <em>0</em></span>"+
						"</p>";
			}
		...
		countClass:function(){				//计算选择的类别及钱数
			...
			var _price=parseInt(_this._service_charge);
